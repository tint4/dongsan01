...
function renderBlogPost(result) {
  ...
  blogPost.innerHTML = `
    ...
    <p class="post-meta">${searchedDate} 공항리무진 조회 기준</p>
...
async function searchBusNo(busNo) {
  ...
  try {
    setStatus(`${busNo}번 공항리무진 시간표를 조회하고 있습니다.`);
    const result = await apiGet(`/api/airport-limousine/search?busNo=${encodeURIComponent(busNo)}`);
    renderBlogPost(result);
    setStatus(`${result.busNo}번 공항리무진 시간표를 만들었습니다.`);
  } catch (error) {
...
async function loadRouteButtons() {
  try {
    ...
    const data = await apiGet("/api/airport-limousine/routes");
