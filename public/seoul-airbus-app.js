...
function renderRows(rows) {
  if (!rows.length) return '<tr><td colspan="6">조회된 시간이 없습니다.</td></tr>';
  return rows
    .map(
      (row) => `
        <tr>
...
function renderBlogPost(result) {
  ...
  blogPost.innerHTML = `
    ...
    <h2 class="post-title">서울공항리무진 ${result.busNo}번 시간표</h2>
    <p class="post-meta">${searchedDate} 서울공항리무진 조회 기준</p>
...
async function searchBusNo(busNo) {
  ...
  try {
    setStatus(`${busNo}번 서울공항리무진 시간표를 조회하고 있습니다.`);
    const result = await apiGet(`/api/seoul-airbus/search?busNo=${encodeURIComponent(busNo)}`);
    renderBlogPost(result);
    setStatus(`${result.busNo}번 서울공항리무진 시간표를 만들었습니다.`);
  } catch (error) {
...
async function loadRouteButtons() {
  try {
    ...
    const data = await apiGet("/api/seoul-airbus/routes");
